      ...
      <div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[1.1fr_0.9fr]">
        <section className="hidden rounded-[2rem] border border-white/10 bg-white/5 p-10 text-white shadow-2xl lg:block">
          ...
          <p className="mt-6 max-w-lg text-lg text-slate-200">
            Sign in to manage the dashboard. Public widget traffic stays separate from internal operator access.
          </p>
          <div className="mt-12 grid gap-4 text-sm text-slate-200">
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4">Email and password login</div>
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4">Google sign-in when configured</div>
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4">Password reset flow for operator recovery</div>
          </div>
        </section>
      ...
      <form className="space-y-5" onSubmit={handleSubmit}>
        ...
        <label className="block">
          <div className="mb-2 flex items-center justify-between">
            ...
            <Link className="text-sm font-medium text-sky-700 hover:text-sky-800" to="/forgot-password">
              Forgot password?
            </Link>
          </div>
          <input
